...
public class UserDAO {
   ...
   public static Long userId; //login 상태, 회원정보 수정 등에서 사용하기 위해서 선언함 
...
//   추천수
   public Long getCountOfRecommenderId(Long userId) { //'내'가 추천을 받은 횟수이므로 매개변수에 session(userId)를 넣고, userId 타입인 long을 반환 
//      String query = "SELECT RECOMMEND_COUNT FROM TBL_USER WHERE USER_ID = ?";
      String query = "SELECT RECOMMEND_COUNT FROM TBL_RECOMMEND WHERE USER_ID = ?"; //쿼리: 추천횟수를 반환, 별도로 만든 rec 테이블에 user_id를 넣으면 
      Long recommendCount = 0L; //추천카운트 초기화 
      connection = DBConnecter.getConnection(); //db 연결 
      try {
         preparedStatement = connection.prepareStatement(query); //query문 연결
         preparedStatement.setLong(1, userId); //userId를 넣음 
         resultSet = preparedStatement.executeQuery(); //select이므로 executeQeury로 결과값을 받음
         
         if(resultSet.next()) { //이 결과값이 true 일 경우에는 (추천을 받았을 경우에는??)
            recommendCount = resultSet.getLong(1); //rec카운트는 getlong 1을 세팅한다.
         }
      } catch (SQLException e) { //trycatch문 
         System.out.println("getCountOfRecommenderId(Long) SQL문 오류"); //오류발생 시 sql문 오류 
         e.printStackTrace();
      } catch (Exception e) {
         e.printStackTrace();
      } finally {
      ...
      return recommendCount; //try catch 이후 recommendCount를 반환한다 
   }
   
//   나를 추천한 사람
   public ArrayList<UserVO> selectRecommenders(){ //userVO를 반환, arrayList 
      String query = "SELECT USER_ID, USER_IDENTIFICATION, USER_NAME, USER_PASSWORD,"
            + " USER_PHONE, USER_NICKNAME, USER_EMAIL, USER_ADDRESS, USER_BIRTH, "
            + "USER_GENDER, USER_RECOMMENDER_ID FROM TBL_USER WHERE USER_RECOMMENDER_ID = ?"; //'나'를 추천한 사람이므로 모든 정보가 필요, ?는 어떤 아이디를 기입햇는지
      UserVO userVO = null; //uservo를 받기 위해 초기화 
      ArrayList<UserVO> users = new ArrayList<UserVO>(); // user 생성자 만들기 
      
      connection = DBConnecter.getConnection(); //db커넥트 연결 
      try {
         preparedStatement = connection.prepareStatement(query); //query 문 실행
         preparedStatement.setString(1, select(userId).getUserIdentification()); //실제로 기입한 아이디를 받음 
         
         resultSet = preparedStatement.executeQuery();  //result에 query 값을 날림 
         
         while(resultSet.next()) { //얼마나 많은 수가 나를 추천할지 모르기 때문에 while 문 사용, result 값이 true 일 경우
            userVO = new UserVO(); //userVO 선언 
            userVO.setUserId(resultSet.getLong(1)); 
            userVO.setUserIdentification(resultSet.getString(2));
            userVO.setUserName(resultSet.getString(3));
            userVO.setUserPassword(resultSet.getString(4));
            ...
            userVO.setUserBirth(resultSet.getString(9));
            userVO.setUserGender(resultSet.getString(10));
            userVO.setUserRecommenderId(resultSet.getString(11)); //'나'를 추천한 사람 정보 세팅
            users.add(userVO); //userVO 값에 users 생성자를 더함 
         }
         
      } catch (SQLException e) {
         System.out.println("selectRecommenders() SQL문 오류"); //오류 발생 시
         e.printStackTrace();
      } catch (Exception e) {
         e.printStackTrace();
      } finally {
      ...
      return users; //나를 추천한 사람 반환 
   }
   
//   내가 추천한 사람
   public UserVO getMyRecommender() { //내가 추천한 사람엔 UserVO 클래스를 반환 
      String query = "SELECT USER_ID, USER_IDENTIFICATION, USER_NAME, USER_PASSWORD,"
            + " USER_PHONE, USER_NICKNAME, USER_EMAIL, USER_ADDRESS, USER_BIRTH, "
            + "USER_GENDER, USER_RECOMMENDER_ID FROM TBL_USER WHERE USER_IDENTIFICATION  = " //쿼리이이이이 
            + "("
      ...
      connection = DBConnecter.getConnection(); //db 연결 
      try {
         preparedStatement = connection.prepareStatement(query); //쿼리문 실행
         preparedStatement.setString(1, select(userId).getUserIdentification());
         
         resultSet = preparedStatement.executeQuery(); //select값이므로 result값을 반환 
         
         if(resultSet.next()) { //resultSet이 true이면
            userVO = new UserVO();
            userVO.setUserId(resultSet.getLong(1));
            userVO.setUserIdentification(resultSet.getString(2));
            userVO.setUserName(resultSet.getString(3));
